...
A plugin to simplify loading the [Monaco Editor](https://github.com/Unity-Billal-mesloub/monaco-editor) with [webpack](https://webpack.js.org/).
...
- `filename` (`string`) - custom filename template for worker scripts, respects the same options as [loader-utils' interpolateName](https://github.com/Unity-Billal-mesloub/loader-utils#interpolatename). Useful for adding content-based hashes so that files can be served with long-lived caching headers.
